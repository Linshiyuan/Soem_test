...
static int slave_dc_config(uint16 slave)
{
    ...
    // ec_dcsync0(slave, TRUE, 1000 * 1000, 0 * 1000);
    ...
    if (ec_init(ifname))
    {
        ...
        if (ec_config_init(FALSE) > 0) // ec_config(FALSE, &IOmap) > 0
        {
            ...
            for (cnt = 1; cnt <= ec_slavecount; cnt++)
            {
                ...
                WRITE(cnt, 0x60c2, 1, buf8, 0x01, "cyc time");
                // ec_dcsync0(cnt, TRUE, 1000 * 1000, 0 * 1000);
                // ec_dcsync01(cnt, TRUE, 1000 * 1000, 1000 * 1000, 0); // SYNC0/1 on slave 2
            }
            ...
            for (cnt = 1; cnt <= ec_slavecount; cnt++)
            {
                printf("Slave:%d Name:%s Output size:%3dbits Input size:%3dbits State:%2d delay:%d.%d\n", cnt,
                       ec_slave[cnt].name, ec_slave[cnt].Obits, ec_slave[cnt].Ibits, ec_slave[cnt].state,
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL)
            {
                ...
                imu_write=(imuOut *)(ec_slave[1].outputs);
                imu_Read=(imuIn *)(ec_slave[1].inputs);
                target = (out_ISMC *)(ec_slave[2].outputs);
                val = (in_ISMC *)(ec_slave[2].inputs);
                target2 = (out_ISMC *)(ec_slave[3].outputs);
                val2 = (in_ISMC *)(ec_slave[3].inputs);
                target3 = (out_ISMC *)(ec_slave[4].outputs);
                val3 = (in_ISMC *)(ec_slave[4].inputs);
                target->opMode = 9;
                target2->opMode = 9;
                target3->opMode = 9;
                ...
                for (i = 1; ; i++)//i <= 50000
                {
                    printf("Processdata cycle %5d , Wck %3d, DCtime %12ld, dt %12ld", dorun, wkc, ec_DCtime,
                           gl_delta);
                    // for (j = 0; j < 16; j++)
                    // {
...
void ec_sync(int64 reftime, int64 cycletime, int64 *offsettime)
{
    ...
    *offsettime = -(delta / 200) - (integral / 40);
    gl_delta = delta;
}
    ...
    while (1)
    {
        ...
        if (dorun > 0)
        {
            ...
            if (inOP)
            {
                ...
                {
                    target->status = 0x1f;
                    // target->tVel = -(int32)(sin(i / 5000.) * (200000));
                      target->tVel=(int32)(imu_Read->roll*100000);
                }
                ...
                {
                    target2->status = 0x1f;
                    target2->tVel = -(int32)(sin(i / 5000.) * (200000));
                        target2->tVel=(int32)(imu_Read->roll*100000);
                }

                uint16 cur_status3 = val3->status; // 0x6041
                if ((cur_status3 & command3) == 0x0040)
                {
                    target3->status = 0x06; // 0x6040
                    // set control mode
                    command3 = 0x006F;
                }
                else if ((cur_status3 & command3) == 0x0021)
                {
                    target3->status = 0x07;
                    command3 = 0x006F;
                }
                else if ((cur_status3 & command3) == 0x0023)
                {
                    target3->status = 0x0f;
                    command3 = 0x006F;
                }
                else if ((cur_status3 & command3) == 0x0027)
                {
                    target3->status = 0x1f;
                    target3->tVel = -(int32)(sin(i / 5000.) * (200000));
                        target3->tVel=(int32)(imu_Read->roll*100000);
                }
            }
            if (ec_slave[0].hasdc)
            {
